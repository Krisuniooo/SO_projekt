		...
		for(int i = 0; i<PRODUCTS; i++) {
			...
			if(SEMAPHORE::lock(data->trays[i].sem_num)) {
				...
				if((data->trays[i].in_stock + amount) > Products_base[i].max_stock) {
					...
					std::string log_message = "Baker: added " + std::to_string(Products_base[i].max_stock - data->trays[i].in_stock) + " " + Products_base[i].label + ", new value: " + std::to_string(data->trays[i].in_stock + amount) + "\n";
